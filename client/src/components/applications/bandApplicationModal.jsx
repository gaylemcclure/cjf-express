...
// import {ClickButton,SmallButton,SmallBackButton,} from "@/app/components/button";
...
const BandApplicationModal = () => {
  ...
  const [pages, setPages] = useState<string>("start");
  //Button disabled bools
  const [contactDisabled, setContactDisabled] = useState<boolean>(true);
  const [datesDisabled, setdatesDisabled] = useState<boolean>(true);
  const [availabilityDisabled, setavailabilityDisabled] = useState<boolean>(true);
  const [feesDisabled, setfeesDisabled] = useState<boolean>(true);
  const [scheduleDisabled, setscheduleDisabled] = useState<boolean>(true);
  //Verification bools
  const [playedBefore, setPlayedBefore] = useState<boolean>(false);
  const [dataRetrieved, setDataRetrieved] = useState<boolean>(false);
  const [inputId, setInputId] = useState<string>();
  const [verified, setVerified] = useState<boolean>(false);
  //Band data
  const [allBandData, setAllBandData] = useState<any>();
  const [bandName, setBandName] = useState<string>("");
  const [leaderName, setLeaderName] = useState<string>("");
  const [leaderEmail, setLeaderEmail] = useState<string>("");
  const [leaderPhone, setLeaderPhone] = useState<number>();
  const [bandId, setBandId] = useState<string>();
  const [bandStyle, setBandStyle] = useState<string>();
  const [availability, setAvailability] = useState<string>();
  const [websiteUrl, setWebsiteUrl] = useState<string>();
  const [leaderInstrument, setLeaderInstrument] = useState<string>();
  const [musicians, setMusicians] = useState<string>();
  const [bio, setBio] = useState<string>();
  const [firstFee, setFirstFee] = useState<string>();
  const [secondFee, setSecondFee] = useState<string>();
  const [numberMembers, setNumberMembers] = useState<string>();
  const [upload, setUpload] = useState<string>();

  let bandInputData: any;
    ...
    <div>
      ...
      >
        ...
        <ModalDialog sx={{ width: 800 }}>
          {pages === "start" && (
            <>
              <AppQ.FirstQuestion
                ...
                click={handlePage}
              />
              <button onClick={() => setPages("questions")}>Skip Question Hack</button>
            </>
          )}
          {pages === "contact" && (
            <AppQ.Questions
          ...
          {pages === "questions" && (
            <>
              <h1 className="justify-center flex uppercase text-black text-4xl font-extrabold pb-8">Band Details</h1>

              <Input.TextInput
                label="Band Name"
                id="band-name"
                txtname="band-name"
                change={(e: any) => setBandName(e.target.value)}
              ...
              <Input.EmailInput
                ...
                id="band-email"
                emlname="band-email"
                change={(e: any) => setLeaderEmail(e.target.value)}
              />
              <div>
                <div className="flex">
                  <p className="mr-4 block text-sm font-medium leading-6 text-gray-900">Have you applied for a previous festival ? </p>
                  <div className={`${styles.help_tip}`}>
                    <p className="">
                      If you have applied previously, we may be able to retrieve saved information to help complete your
                      application.
                    </p>
                  </div>
                </div>
                <FormControlLabel
                  ...
                  control={<Switch checked={playedBefore} onChange={() => setPlayedBefore(!playedBefore)} />}
                />
              </div>
              ...
              {playedBefore === true && (
                <>
                  <div className="flex ">
                    <div className="mr-4">Retrieve previous application data?</div>
                    <div className={`${styles.help_tip}`}>
                      <p>
                        Please check your email address is correct, and we will send you a code via email to retrieve
                        your details.
                      </p>
                    </div>
                  </div>
              ...
              {dataRetrieved === true && (
                <>
                  <p>
                    If the Band Name, Band Leader Name and Band Leader Email match an application in the system, an
                    email has been sent to your selected email address with a code. Input the code in the Band Id field
                    and click retrieve to auto populate your application. You can update the information provided and
                    this will be saved for your new application
                  </p>
                  <Input.TextInput
                label="Band Leader Name"
                id="band-leader"
                txtname="band-leader"
                change={(e: any) => setLeaderName(e.target.value)}
              ...
              />
                  <Input.TextInput
                    ...
                    id="band-id"
                    txtname="band-id"
                    change={(e: any) => setInputId(e.target.value)}
              ...
              {verified === true && (
                <>
                  <Input.ValueTextInput
                    ...
                    id="band-phone"
                    txtname="band-phone"
                    change={(e: any) => setLeaderPhone(e.target.value)}
                  ...
                  <Input.ValueTextInput
                    ...
                    id="band-style"
                    txtname="band-style"
                    change={(e: any) => setBandStyle(e.target.value)}
                  ...
                  <Input.ValueTextInput
                    ...
                    change={(e: any) => setLeaderInstrument(e.target.value)}
                  ...
                  <Input.ValueTextInput
                    ...
                    change={(e: any) => setAvailability(e.target.value)}
                    value=""
                  />
                  <Input.ValueTextInput
                    label="Band Website"
                    id="band-url"
                    txtname="band-url"
                    change={(e: any) => setWebsiteUrl(e.target.value)}
                  ...
                  <Input.ValueTextInput
                    ...
                    id="musicians"
                    txtname="musicians"
                    change={(e: any) => setMusicians(e.target.value)}
                    value=""
                  />
                  <Input.ValueTextInput
                    label="Band Bio"
                  ...
                  <Input.ValueTextInput
                    ...
                    id="first-fee"
                    txtname="first-fee"
                    change={(e: any) => setFirstFee(e.target.value)}
                    value=""
                  />
                  <Input.ValueTextInput
                    ...
                    id="second-fee"
                    txtname="second-fee"
                    change={(e: any) => setSecondFee(e.target.value)}
                    value=""
                  />
                  <Input.ValueTextInput
                    ...
                    id="band-img"
                    txtname="band-img"
                    change={(e: any) => setUpload(e.target.value)}
                    value=""
                  />
                </>
              )}
