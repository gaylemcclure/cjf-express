...
import { MEMBER_CONFIRMATION, MEMBER_CONFIRMATION_CJF } from "../utils/mutations";
...
const MemberSuccess = () => {
  ...
  const [sendMembershipConfirmationMember] = useMutation(MEMBER_CONFIRMATION);
  const [sendMembershipConfirmationCJF] = useMutation(MEMBER_CONFIRMATION_CJF);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const res = await axios.get("/api/page/member-email");
        const data = res;
        setEmailData(data.data.items[0].fields);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  useEffect(() => {
    const fName = new URLSearchParams(window.location.search).get("name");
    const memEmail = new URLSearchParams(window.location.search).get("email");
    if (!stripe) {
      return;
    }
  ...
  const handleContactEmail = async () => {
    try {
      const { data } = await sendMembershipConfirmationMember({
        variables: {
          senderEmail: email,
          senderName: name,
          firstParagraph: emailData.firstParagraph.content[0].content[0].value,
          from: emailData.from,
          fromName: emailData.fromName,
          personalisation: emailData.personalisation,
  ...
  const handleAdminEmail = async () => {
    try {
      const { data } = await sendMembershipConfirmationCJF({
        variables: {
          senderEmail: email,
          senderName: name,
          year: year,
          ...
          {intentId && (
            <div id="details-table">
              <table>
                <tbody>
                  <tr>
                    <td className="TableLabel">Thank you for becoming a member of the Castlemaine Jazz Festival.</td>
                  </tr>
                </tbody>
              </table>
            </div>
